function  [ time adjusted_close ] = StockRegression( CSV_Filename )
...
%generate a polynomial regression that acts as an average
degree = 3;
xValue = linspace(time(1),time(length(time)));
coefficient = polyfit(time,adjusted_close,degree);
yValue = polyval(coefficient, xValue);
hold on
